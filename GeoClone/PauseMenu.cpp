...
PauseMenu::PauseMenu(std::shared_ptr<SoundManager> &soundManager) :
	MenuState(soundManager)
{
	//Spaces to align text, may be quicker than calculating their positions...
	_menuItems.push_back(" Continue");
	_menuItems.push_back("Exit Game");
	_startX = 400;
	_startY = 200;
	_selectedItem = 0;
	MenuState::getSoundManager()->pauseMusic();
}
...
void PauseMenu::update(std::shared_ptr<Draw> &renderer){
...
void PauseMenu::changeItem(int change){
